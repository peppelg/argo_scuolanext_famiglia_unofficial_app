...
class _VotiRouteState extends State<VotiRoute> {
  ...
  Widget build(BuildContext context) {
    ...
    var mediaPerMese = {
      ...
      'ScrittoOrale': { 'value': List.filled(12, .0), 'count': List.filled(12, .0) }
    };
    ...
    var mediaGlobale = sommaGlobale.map((key, value) =>
      new MapEntry(key, (value['count'] != 0 ? (value['value'] / value['count']) : 0).toStringAsFixed(2))
    );
    ...
    var widgetsMediaGlobale = ExpansionTile(
          ...
          title: Text("MEDIA GENERALE", style: new TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          ...
          children: <Widget>[
            ...
            Row(
              ...
              children: [
                ...
                Expanded(
                    child: Column(children: <Widget>[
                      cerchioVoto(mediaGlobale['ScrittoOrale'].toString(), radius: 60.0),
                      Opacity(opacity: 0.6, child: Text('Media totale'))
                ]))
              ]
            ),
    ...
    return BackdropScaffold(
        ...
        frontLayer: RefreshIndicator(
            ...
            child: ListView(
                children: voti.isEmpty ? <Widget>[] : <Widget>[
                  widgetsMediaGlobale,
    ...
    for (var voto in listaVoti) {
      if (!voto['commento'].contains('non fa media') &&
        ...
        if (medie.containsKey(voto['tipo'])) {
          ...
          mediePerMese['ScrittoOrale']['value'][meseVoto] += double.parse(voto['voto'].toString()) * valore;
          mediePerMese['ScrittoOrale']['count'][meseVoto] ++;

          mediePerMese[voto['tipo']]['value'][meseVoto] += double.parse(voto['voto'].toString()) * valore;
